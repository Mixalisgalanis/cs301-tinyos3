...
Pid_t sys_Exec(Task call, int argl, void *args) {
    ...
    /*
      ...
      thing we do, because once we wakeup the new thread it may run! so we need to
    ...
    if (FCB_reserve(1, &fid, &fcb) == 1) {
        ...
        for (int i = 0; i < MAX_PROC; i++) {
            if (PT[i].pstate != FREE) {
                ...
                prinfo->ppid = (PT[i].parent == 0) ? 0 : get_pid(PT[i].parent);
                prinfo->alive = (PT[i].pstate == ALIVE) ? 1 : 0;
                prinfo->main_task = PT[i].main_task;
                prinfo->argl = PT[i].argl;

                int max = (PT[i].argl < PROCINFO_MAX_ARGS_SIZE) ? PT[i].argl : PROCINFO_MAX_ARGS_SIZE;

                for (int j = 0; j < max; j++) {
                    prinfo->args[j] = ((char *) PT[i].args)[j];
                }
...
int system_info_read(void *prin_cb, char *buffer, unsigned int size) {
    ...
    PICB *picb = (PICB *) prin_cb;
    for (int i = 0; i < size; i++) {
        if (picb->reader >= picb->writer){
            return i;
        } else{
            buffer[i] = picb->buffer[picb->reader++];
        }
    }
    return size - 1;
}
